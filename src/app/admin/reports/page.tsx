...
export default function ReportsPage() {
  ...
  const handleExport = async (formatType: 'pdf' | 'excel') => {
    ...
    const tableData = filteredTickets.map(ticket => [
        ...
        ticket.name,
        ticket.institucion || 'N/A',
        ticket.ciudad || 'N/A',
        ticket.tipo_incidente || 'N/A',
        ticket.status,
        ticket.assignedTo || 'Sin Asignar',
        format(new Date(ticket.createdAt), "dd/MM/yyyy HH:mm"),
        ticket.resolvedBy || 'N/A',
        ticket.resolvedAt ? format(new Date(ticket.resolvedAt), "dd/MM/yyyy HH:mm") : 'N/A',
        ticket.resolution_details || 'N/A',
        ticket.pending_activities || 'N/A',
        ticket.resolution_parts || 'N/A',
        ticket.observations || 'N/A',
    ]);
    const tableHeaders = ["ID", "Cliente", "Institución", "Ciudad", "Incidente", "Estado", "Técnico", "Fecha Creación", "Resuelto Por", "Fecha Resolución", "Actividades Realizadas", "Act. Pendientes", "Repuestos", "Observaciones"];
        ...
        if (statusChartRef.current && incidentChartRef.current && (ticketStatusData.length > 0 || incidentTypeData.length > 0)) {
            try {
                ...
                if (incidentTypeData.length > 0) {
                    doc.addImage(incidentImgData, 'PNG', pdfWidth / 2 + 5, 30, chartWidth, incidentHeight);
                }
            } catch (error) {
    ...
    if (formatType === 'excel') {
        const worksheetData = filteredTickets.map(ticket => ({
            ...
            "Resuelto Por": ticket.resolvedBy || 'N/A',
            "Fecha Resolución": ticket.resolvedAt ? format(new Date(ticket.resolvedAt), "dd/MM/yyyy HH:mm") : 'N/A',
            "Actividades Realizadas": ticket.resolution_details || 'N/A',
            "Actividades Pendientes": ticket.pending_activities || 'N/A',
            "Repuestos Utilizados": ticket.resolution_parts || 'N/A',
            "Observaciones": ticket.observations || 'N/A',
        }));
        ...
        XLSX.writeFile(workbook, `reporte_actividades_${format(new Date(), "yyyy-MM-dd")}.xlsx`);
    }

    setIsExporting(false);
    toast({
        title: "Exportación Exitosa",
        description: `El reporte se ha generado en formato ${formatType.toUpperCase()}.`,
    });
  };
    ...
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
            ...
            <p className="text-muted-foreground">Filtra por fecha para ver reportes diarios o visualiza el historial completo.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card ref={incidentChartRef}>
          <CardHeader>
            <CardTitle>Incidentes Más Comunes</CardTitle>
